...
2. 不同操作系统的换行符 (EOL) 有什么区别?
  ...
  1. 数据持久化
      ...
      * 相比于AOF机制，如果数据集很大，RDB的启动效率会更高。
      ...
      2. 单线程的redis为什么这么快 (核心原因是避免多线程开发的并发控制问题)
      ...
      3. redis的数据恢复
        * AOF日志 redis在命令执行之后会记录一条写后日志
          1. 这样所有被写入日志的命令都是能够执行成功的 但是可能会在宕机前没有把写日志 导致数据丢失
          2. 在命令执行后才记录日志，不会阻塞当前的写操作 但是会阻塞下一个命令。 
        * ROB内存快照  
          save 与 bgsave Redis默认使用bgsave与写时复制技术一起来保证快照数据的完整性。  
          那么多久执行一次快照呢 一般进行一次去全量快照之后在之后使用AOF记录两次快照之间的操作日志
        * 总结 
          1. 数据不能丢失时，内存快照和 AOF 的混合使用是一个很好的选择；
          2. 如果允许分钟级别的数据丢失，可以只使用 RDB；
          3. 如果只用 AOF，优先使用 everysec 的配置选项，因为它在可靠性和性能之间取了一个平衡。

      4. redis的可靠性依赖 1. 数据少丢失（使用数据恢复） 2. 服务少中断（使用主从集群） 。 
        * 主从库 写都在主库上 从库负责同步主库的数据与读操作
        * 主库挂了 如何不间断服务? 
        > Redis 主从集群中，哨兵机制是实现主从库自动切换的关键机制，它有效地解决了主从复制模式下故障转移的这三个问题。哨兵负责监控所有的节点的健康性， 当发现主库挂掉之后立马切换一个从库为主库并且告诉其它从库主库的位置。 其它从库又重新与主库建立新的主从关系。 

        > Redis 切片集群 - 按照hash slot的方式将键分配到不同的实例上面

      5. redis实践过程中的坑
        * string类型其实比较占用空间 
        * 统计的时候需要好好的借用redis的数据设计 比如set的交集 并集等
          * 聚合统计 使用set
          * 排序统计 list 与 sorted set
          * 二值状态统计 bitmap （string）如果只需要统计数据的二值状态，例如商品有没有、用户在不在等，就可以使用 Bitmap，因为它只用一个 bit 位就能表示 0 或 1。在记录海量数据时，Bitmap 能够有效地节省内存空间。
          * 基数统计 比如uv需要对用户去重 然后scard来统计 但是存在如果又1亿用户会占用很大的空间 所以我们使用[HyperLogLog](https://www.runoob.com/redis/redis-hyperloglog.html)
        
      6. 使用redis的GEO来实现附近的mm
      ```
        GEOADD cars:locations 116.034579 39.030452 33 // 将编号为33的元素到这个cars:lacations里面

        GEORADIUS cars:locations 116.054579 39.030452 5 km ASC COUNT 10 // 寻在在某个坐标点周围5km内的10个元素
      ```
      ...
      8. redis 的主线程阻塞
        ![alt redis问题总结](./redis/block.jpg)
        * 客户端
          * 网络交互 - 多路复用机制
          * 键值对的增删改 - 复杂度为o(N)的操作 1. 集合全量查询和聚合操作 2. bigkey的删除操作 3. 清空数据库
        * 磁盘交互
          > 一般使用子进程生产RDB快照文件 但是AOF日志的同步写会阻塞
        * 
      9. CPU对redis速度的影响
      ...
      8. 如何应对波动的响应延迟
        * 寻找慢查询命令 并优化 
        * 对EXPIREAT参数允许随机值的上下波动
      
      9. Redis的缓存
        * Redis 缓存具体是怎么工作的？
          1. 只读缓存 应用要读取数据的话，会先调用 Redis GET 接口，查询数据是否存在。而所有的数据写请求，会直接发往后端的数据库，在数据库中增删改。对于删改的数据来说，如果 Redis 已经缓存了相应的数据，应用需要把这些缓存的数据删除，Redis 中就没有这些数据了。
          2. 读写缓存 除了读请求会发送到缓存进行处理（直接在缓存中查询数据是否存在)，所有的写请求也会发送到缓存，在缓存中直接对数据进行增删改操作。此时，得益于 Redis 的高性能访问特性，数据的增删改操作可以在缓存中快速完成，处理结果也会快速返回给业务应用，这就可以提升业务应用的响应速度 但是我们数据要写到数据库中
            > * 同步直写
            > * 异步写回
          
        * Redis 缓存如果满了，该怎么办？
          > 触发redis的缓存淘汰机制 回自动删除不常用的数据
        * 为什么会有缓存一致性、缓存穿透、缓存雪崩、缓存击穿等异常，该如何应对？
          1. 数据一致性的应对方式 还是要看业务 对于高响应业务 要加全局锁 等待操作完成之后再读取数据 2 先操作数据库 再删除缓存
          2. 缓存雪崩是指大量的应用请求无法在 Redis 缓存中进行处理，紧接着，应用将大量请求发送到数据库层，导致数据库层的压力激增。 
              * 大量数据同时国旗 导致大量请求无法得到处理 - 避免设置相同过期时间 
              * redis缓存实例宕机 - 使用稳定可靠的集群服务 
          3. 缓存击穿 - ，针对某个访问非常频繁的热点数据的请求，无法在缓存中进行处理，紧接着，访问该数据的大量请求，一下子都发送到了后端数据库，导致了数据库压力激增，会影响数据库处理其他请求。缓存击穿的情况，经常发生在热点数据过期失效时。 - 对于热点数据不设置过期时间
          4. 缓存穿透 - 要访问的数据既不在 Redis 缓存中，也不在数据库中，导致请求在访问缓存时，发生缓存缺失，再去访问数据库时，发现数据库中也没有要访问的数据。 - 业务隔离 在redis中设置空值返回
        * 缓存污染 缓存中存在大量无效的数据 - 对所有业务数据都设置合理的过期时间
      
      10. Redis的原子性操作
        * 把多个操作在 Redis 中实现成一个操作，也就是单命令操作； - 比如INC DECR 
        * 把多个操作写到一个 Lua 脚本中，以原子性方式执行单个 Lua 脚本。
      
      11. 基于单个 Redis 节点实现分布式锁

          ```JavaScript
                      async createLock(key: string, expirySecound = 60): Promise<Lock | null> {
                        const { app } = this;
                        const uuid = this.getUuid();
                        const nowDate = new Date();
                        const res = await app.redis.set('LOCK:' + key, `${uuid}|${nowDate}`, 'EX', expirySecound, 'NX');
                        this.ctx.logger.debug('[LOCK_CREATE] k: %s, res: %s', key, res);
                        if (res === 'OK') {
                          return {
                            key,
                            uuid,
        ...
        12. redis 如何实现[事务](https://www.runoob.com/redis/redis-transactions.html)
        
        13. redis 实现秒杀的要务
            > 1. 页面元素静态化 使用cdn等缓存js或者图片
            > 2. 秒杀活动开始
                  
              *   第一步 库存查验 在redis里面原子性对库存减1 如果成功 则用户获得购买资格 用户可以进行后续下单支付金额等操作
              如果失败 则用户无法获取到库存 无法下单 

              *   第二部 用户在支付等过程中取消订单 需要回滚用户的库存 此时使用lua脚本 如果库存大于0则
                设置库存incer 1 如果库存小于0 则设置库存为1
              
              *   如果用户成功下单 这些订单的操作可以在数据中完成 
